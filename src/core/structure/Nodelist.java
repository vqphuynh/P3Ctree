...
public class Nodelist implements INlist {
	...
	private int supportCount = -1;
 	...
 	public String toString(){
 		...
 		sb.append("}, freq:").append(this.supportCount());
 	...
 	public boolean isIdentical(INlist nlist) {
		if (this.size != nlist.size()) return false;
		
		int size = this.size;
		Node node = new Node();
		for (int i=0; i<size; i++){
			nlist.get(i, node);
			if (this.ppc[0][i] != node.pre || this.ppc[1][i] != node.pos || this.ppc[2][i] != node.count) return false;
		}
		return true;
	}
